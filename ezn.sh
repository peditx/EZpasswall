...
# Apply PWA meta tags for iOS and Android support
if [[ -f $THEME_PATH ]]; then
    echo "Argon theme found, applying PWA changes..."
    sed -i '/<head>/a \
    <meta name="apple-mobile-web-app-capable" content="yes"> \
    <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent"> \
    <link rel="apple-touch-icon" href="/luci-static/argon/favicon.ico"> \
    <meta name="mobile-web-app-capable" content="yes"> \
    <link rel="icon" sizes="192x192" href="/luci-static/argon/favicon.ico"> \
    <link rel="manifest" href="/luci-static/argon/manifest.json"> \
    ' $THEME_PATH
fi
...
if grep -q '<meta name="apple-mobile-web-app-capable" content="yes">' "$THEME_PATH"; then
    echo -e "${GREEN}PWA changes applied successfully ✅ OK${NC}"
else
    echo -e "${RED}PWA changes failed ❌ FAILED${NC}"
fi
